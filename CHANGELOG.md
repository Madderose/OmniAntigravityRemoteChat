...
- 🧪 **Simulated End-to-End Test Suite** — Added `test/unit/simulated-e2e-workflows.test.js` (27 automated tests) covering LAN auto-auth IP boundaries, workspace symlink confinement, terminal execution lifecycle & orphan termination, `withSendLock` concurrency serialization, quota service offline resilience, Lexical staging boundary normalization, upload size & MIME constraints, and deep readiness probing.
- 🩺 **Deep Readiness & System Health Probe** — Added `GET /ready` and `GET /health/deep` endpoints providing real-time diagnostics on V8 heap memory usage, active CDP connectivity, `withSendLock` queue backlog, and storage directory writeability with automated HTTP 200/503 status.
- 🚀 **Expanded CI Matrix & Test Pipeline** — Upgraded `.github/workflows/ci.yml` to trigger on `feat/**` branches and execute the complete Vitest unit test suite alongside smoke integration tests.

### Fixed
- 📝 **Lexical Editor Multiline & Clipboard Staging** (`TICKET-DOM03-001`, `TICKET-DOM03-002`, P1/P2) — Refactored message injection to utilize synthetic `ClipboardEvent` with `DataTransfer` (`text/plain`), triggering Lexical's native `PASTE_COMMAND` to cleanly generate paragraph nodes without AST desync, supplemented by an 85% character normalization boundary validation.
- 🔒 **Comprehensive API Route Auth Guard** (`TICKET-DOM06-004`, P2) — Enforced strict HTTP 401 JSON authentication across all 60+ operational endpoints (`/api/*`, `/cdp-*`, `/remote-*`, `/select-*`, `/new-chat`, `/chat-*`, `/app-state`) regardless of request `Accept` headers.
- 📦 **Media Upload Boundaries & MIME Whitelisting** (`TICKET-DOM08-002`, P2) — Enforced 15 MB maximum payload limits (`MAX_IMAGE_BYTES = 15 * 1024 * 1024`), rejected empty buffers, and whitelisted valid image/audio MIME types.
- 🤖 **Telegram Command Isolation & Auth Guard** (`TICKET-DOM12-002`, P2) — Hardened Telegram bot command dispatch with strict user validation against `TELEGRAM_ALLOWED_USERS`.
